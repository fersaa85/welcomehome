...
(function ($) {
  ...
  if (browser.mobile) {
    ...
    settings.dragging.enabled = false; // Re-enable overflow on body.
  ...
  if (browser.name == 'ie') {
    // Enable IE mode.
    $body.addClass('is-ie'); // Page widths.
  ...
  if (settings.keyboardShortcuts.enabled) (function () {
    $wrapper // Prevent keystrokes inside excluded elements from bubbling.
    ...
    .on('keydown', function (event) {
      ...
      if (scrolled) {
        ...
        event.stopPropagation(); // Stop link scroll.
  ...
  if (settings.scrollWheel.enabled) (function () {
    ...
    var normalizeWheel = function normalizeWheel(event) {
      ...
      if ('wheelDeltaX' in event) sX = event.wheelDeltaX / -120; // Side scrolling on FF with DOMMouseScroll.
    ...
    $body.on('wheel', function (event) {
      // Disable on <=small.
      if (breakpoints.active('<=small')) return; // Prevent default.

      event.preventDefault();
      event.stopPropagation(); // Stop link scroll.

      $bodyHtml.stop(); // Calculate delta, direction.

      var n = normalizeWheel(event.originalEvent),
          x = n.pixelX != 0 ? n.pixelX : n.pixelY,
          delta = Math.min(Math.abs(x), 150) * settings.scrollWheel.factor,
          direction = x > 0 ? 1 : -1; // Scroll page.
  ...
  if (settings.scrollZones.enabled) (function () {
    var $left = $('<div class="scrollZone left"></div>'),
        ...
        intervalId = null,
        direction,
        activate = function activate(d) {
      // Disable on <=small.
      if (breakpoints.active('<=small')) return; // Paused? Bail.

      if (paused) return; // Stop link scroll.

      $bodyHtml.stop(); // Set direction.

      direction = d; // Initialize interval.
    ...
    $wrapper.on('---pauseScrollZone', function (event) {
      // Pause.
      paused = true; // Unpause after delay.
  ...
  if (settings.dragging.enabled) (function () {
    var dragging = false,
        ...
        velocity,
        direction;
    $wrapper // Prevent image drag and drop.
    ...
    .on('mouseup mousemove mousedown', settings.excludeSelector, function (event) {
      ...
      event.stopPropagation(); // End drag.

      dragging = false;
      $wrapper.removeClass('is-dragging');
      clearInterval(velocityIntervalId);
      clearInterval(momentumIntervalId); // Pause scroll zone.

      $wrapper.triggerHandler('---pauseScrollZone');
    }) // Mousedown event.
    .on('mousedown', function (event) {
      // Disable on <=small.
      if (breakpoints.active('<=small')) return; // Clear momentum interval.

      clearInterval(momentumIntervalId); // Stop link scroll.

      $bodyHtml.stop(); // Start drag.

      dragging = true;
      $wrapper.addClass('is-dragging'); // Initialize and reset vars.
      ...
      currentX = startX;
      distance = 0;
      direction = 0; // Initialize velocity interval.

      clearInterval(velocityIntervalId);
      velocityIntervalId = setInterval(function () {
        // Calculate velocity, direction.
        velocity = Math.abs(currentX - previousX);
        direction = currentX > previousX ? -1 : 1; // Update previous X.
    ...
    .on('mousemove', function (event) {
      // Not dragging? Bail.
      if (!dragging) return; // Velocity.

      currentX = event.clientX; // Scroll page.

      $document.scrollLeft(startScroll + (startX - currentX)); // Update distance.

      distance = Math.abs(startScroll - $document.scrollLeft()); // Distance exceeds threshold? Disable pointer events on all descendents.
    ...
    .on('mouseup mouseleave', function (event) {
      var m; // Not dragging? Bail.

      if (!dragging) return; // Dragged? Re-enable pointer events on all descendents.
      ...
      if (distance > settings.dragging.threshold) event.preventDefault(); // End drag.
      ...
      $wrapper.triggerHandler('---pauseScrollZone'); // Initialize momentum interval.
      ...
      if (settings.dragging.momentum > 0) {
        ...
        momentumIntervalId = setInterval(function () {
          ...
          $document.scrollLeft($document.scrollLeft() + m * direction); // Decrease momentum.

          m = m * settings.dragging.momentum; // Negligible momentum? Clear interval and end.
  ...
  $wrapper.on('mousedown mouseup', 'a[href^="#"]', function (event) {
    ...
    if (href == '#' || ($target = $(href)).length == 0) return; // Prevent default.

    event.preventDefault();
    event.stopPropagation(); // Calculate x, y.
  ...
  $('.gallery').on('click', 'a', function (event) {
    ...
    setTimeout(function () {
      $modal.removeClass('visible'); // Pause scroll zone.
...
module.exports = __webpack_require__(/*! /Applications/MAMP/htdocs/welcomehome/resources/js/main.js */"./resources/js/main.js");
